...
VS_OUTPUT VS(float4 Pos: POSITION0,float3 Normal: NORMAL,float3 Tangent  : TANGENT0,float2 Texcoord : TEXCOORD0)
{
	...
	float3 T = Tangent;
	float3 B = cross(N,T);
	float3 E = vecEyePos - Pos.xyz;	
	Out.E.x = dot(E,T);
	Out.E.y = dot(E,B);
	Out.E.z = dot(E,N);
	float3 L = -vecLightDir.xyz;		
	Out.L.x = dot(L,T);
	Out.L.y = dot(L,B);
	Out.L.z = dot(L,N);
    ...
    Out.WorldNormal = mul(normal, matWorldInverseTranspose).xyz;
    float4 Po = float4(Pos.xyz,1);
    float3 Pw = mul(Po, matWorld).xyz;
    Out.LightVec = normalize(LightPos - Pw);
    Out.TexCoord = Texcoord;
    Out.WorldView = normalize(matViewInverse[3].xyz - Pw);
...
{   
	...
	f3Sight=In.E- vecEyePos;
	fK=0.1;	
	
	float4 f4Intensity=dot(f3Light,f3Normal) //ディフューズ項
			+ pow(dot(f3Reflect,f3Sight),18);// スペキュラー項(照明モデルは　フォン phong)
			
			f4Color=float4(0.6,0.6,1,1);//青
			f4Intensity*=f4Color;
