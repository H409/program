#include "mesh_skydome.h"
//=============================================================================
//[スカイドームクラス]
// CSkydome.cpp
// Author : 弓長 慧勇 2014/05/21
//
//=============================================================================
Skydome::~Skydome()
{
...
Skydome* Skydome::Create(LPDIRECT3DDEVICE9 device,float posX,float posY,float posZ,float rotY,
							int nNumBlockX,int nNumBlockY,float fSizeBlockX,float fSizeBlockY)
{

	Skydome* p=new Skydome(device);
	p->Init(posX,posY,posZ,rotY,nNumBlockX,nNumBlockY,fSizeBlockX,fSizeBlockY);
	p->SetPosition(0.0f,0.0f,0.0f);
	return p;

}
HRESULT Skydome::Init(	float posX,float posY,float posZ,float rotY,
							int nNumBlockX,int nNumBlockY,float fSizeBlockX,float fSizeBlockY)
{
	VERTEX_3D *pVtx;			//頂点情報
	...
	//シリンダーの面一枚の角度
	float facedigree=(D3DX_PI*2)/nNumBlockX;
	...
	//テクスチャの読み込み
	D3DXCreateTextureFromFile(GetDevice(),"data/TEXTURE/space.jpg",&g_pD3DTexture);

	//頂点座標の設定
	g_pD3DVtxBuff->Lock(0,0,(void**)&pVtx,0);
	for(int y=0,num=0;y<nNumBlockY+1;y++)
	{
		float dr = m_radius*((float)y/(float)nNumBlockY)*((float)y/(float)nNumBlockY);
		float yh = ((float)y/(float)nNumBlockY)*((float)y/(float)nNumBlockY);
		float yc = nNumBlockY*fSizeBlockY/2;

		for(int x=0;x<nNumBlockX+1;x++)
		{
			pVtx[num].vtx = D3DXVECTOR3(-(cosf(facedigree*(float)x))*dr,(-fSizeBlockY*y)*yh+yc,(sinf(facedigree*(float)x))*dr);

			pVtx[num].nor = D3DXVECTOR3(0.0f,1.0f,0.0f);
			pVtx[num].nor = D3DXVECTOR3(0.0f,1.0f,0.0f);
			pVtx[num].diffuse = D3DXCOLOR(1.0f,1.0f,1.0f,1.0f);
			pVtx[num].tex = D3DXVECTOR2((float)x/nNumBlockX,(float)y/10);
			num++;
		}
	}
...
//アクセサー
void Skydome::SetPosition(D3DXVECTOR3 pos)
{
	m_pos=pos;
}
void Skydome::SetPosition(float x,float y,float z)
{
	m_pos= D3DXVECTOR3(x,y,z);
}
void Skydome::SetRotation(D3DXVECTOR3 rot)
{
	m_rot=rot;
}
void Skydome::SetRotation(float x,float y,float z)
{
